...
int         handlepwad(char *fname)
{
    ...
    wad_uint32_t        ndirent;        /* Number of entries in WAD directory */
    wad_uint32_t        diroffs;        /* Offset of directory in WAD file */

    wad_uint32_t        eoffs, elen;    /* Offset and length of a directory entry */
    ...
    fread(&ndirent, sizeof(wad_uint32_t), 1, f);/* Number of entries in WAD dir */
    fread(&diroffs, sizeof(wad_uint32_t), 1, f);/* Offset of directory in WAD */
    ...
    for (i = 0; i < ndirent; i++) {
        ...
        fread(&eoffs, sizeof(wad_uint32_t), 1, f);      /* Offset of entry's data */
        fread(&elen, sizeof(wad_uint32_t), 1, f);       /* Length of entry's data */

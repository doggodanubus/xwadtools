...
******************************************************************************/
#if defined(ANSI_C)
int blockmap_make(short **blockmap, WAD_LINE *lines, int nlines,
                   WAD_VERT *verts)
#else
int blockmap_make(blockmap,lines,nlines,verts)
...
#endif
{
  ...
  for (l = 0; l < nlines; l++) {		/* scan LINEDEFS here */
    xf = scl*((int)verts[lines[l].fndx].x-(int)xorig);
    yf = scl*((int)verts[lines[l].fndx].y-(int)yorig);
    xt = scl*((int)verts[lines[l].tndx].x-(int)xorig);
    yt = scl*((int)verts[lines[l].tndx].y-(int)yorig);

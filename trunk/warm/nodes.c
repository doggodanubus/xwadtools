...
******************************************************************************/
#if defined(ANSI_C)
int nodes_make(WAD_NODE **nodes, int *nnodes, WAD_SSECTOR **ssecs,
               int *nssecs, WAD_SEGS **segs, int *nsegs,
               WAD_VERT **verts, int *nverts, WAD_LINE **lines,
               int *nlines, WAD_SIDE **sides, char *genflags)
#else
int nodes_make(nodes,nnodes,ssecs,nssecs,segs,nsegs,verts,nverts,

...
******************************************************************************/
#if defined(ANSI_C)
boolean patch_read(WAD_INFO *winfo, int entry, int resources_needed)
#else
...
******************************************************************************/
#if defined(ANSI_C)
boolean wad_read(WAD_INFO *winfo, int entry, int resources_needed)
#else
...
#endif
{
  ...
  DOOM_LINE *dlines;
  HEXEN_LINE *xlines;
  int dir_offset;				/* new directory offset */
  int type;					/* level data resource type */
  int maxlen = 0;				/* max size of orig entries */

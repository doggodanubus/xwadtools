...
typedef struct REJECT_INFO {
	...
	int *A, *B, *C;			/* lines equation constants */
...
#endif
{
  ...
  tp->x = (int)vf->x + d*((int)vt->x-vf->x)/ndiv;
  tp->y = (int)vf->y + d*((int)vt->y-vf->y)/ndiv;
}
...
#endif
{
  ...
  static int A, B, C;				/* describes los line */
  ...
  if (lvf.x != vf->x || lvt.x != vt->x ||
    ...
    A = -((int)vt->y - vf->y);			/* partition line is */
    B = (int)vt->x - vf->x;			/* Ax + By + C = 0   */
    C = (int)vt->y*vf->x - (int)vt->x*vf->y;
...
#endif
{
  ...
  int scl = 81920/(100+rinfo.blockmap[3]);	/* line following scaling */
  int size = 0x80*scl;				/* block size (map coords) */
  int xcc, xcl;
  int xf,yf, xt,yt;
  int xd, yd;					/* x direction, y direction */
  int m;					/* diagonal line slope */
  int i;
  int p = 0;					/* increment to next block */
  int c = 0;					/* # blocks to consider */
  unsigned short *ub;
  int b;					/* blockmap block # */
  int d1, d2;
  int l;

  for (d1 = 0; !los && d1 <= rinfo.ndiv; d1++) {/* do l1's test points */
    reject_test_point(l1,d1,rinfo.ndiv,&v1);
    xf = scl*((int)v1.x-(int)rinfo.blockmap[0]);
    yf = scl*((int)v1.y-(int)rinfo.blockmap[1]);
    for (d2 = 0; !los && d2 <= rinfo.ndiv; d2++) {/* do l2's test points */
      reject_test_point(l2,d2,rinfo.ndiv,&v2);
      los = TRUE;
      xt = scl*((int)v2.x-(int)rinfo.blockmap[0]);
      yt = scl*((int)v2.y-(int)rinfo.blockmap[1]);
...
******************************************************************************/
#if defined(ANSI_C)
local void reject_init(WAD_LINE *lines, int nlines, WAD_SIDE *sides,
...
#endif
{
  ...
  rinfo.ndiv = ndiv;
  rinfo.A = blockmem(int,rinfo.nlines);
  rinfo.B = blockmem(int,rinfo.nlines);
  rinfo.C = blockmem(int,rinfo.nlines);
  for (l = 0; l < nlines; l++) {		/* compute lines equations */
    vf = &rinfo.verts[rinfo.lines[l].fndx];
    vt = &rinfo.verts[rinfo.lines[l].tndx];
    rinfo.A[l] = -((int)vt->y-vf->y);
    rinfo.B[l] = (int)vt->x-vf->x;
    rinfo.C[l] = (int)vt->y*vf->x - (int)vt->x*vf->y;
  }
  rinfo.nsects = 0;
...
******************************************************************************/
#if defined(ANSI_C)
int reject_make(WAD_REJECT **reject, int genflags, char *except,
                 WAD_LINE *lines, int nlines, WAD_SIDE *sides,
                 WAD_VERT *verts, WAD_BLOCKMAP *blockmap)
#else
int reject_make(reject,genflags,except,lines,nlines,sides,verts,blockmap)
WAD_REJECT **reject;
int genflags;
char *except;
WAD_LINE *lines;
...
#endif
{
  int size;					/* # bytes in REJECT block */
  int s1, s2;
  int l1, l2;
  ...
  size = ((int)rinfo.nsects*rinfo.nsects+7)/8;	/* # bytes in REJECT block */

...
typedef unsigned int themebits;  /* Bitarray, really */
...
typedef unsigned int propertybits;  /* Another bitarray */
#define FLOOR (0x01)
#define CEILING (0x02)
#define DOOR (0x04)
...
slink *random_link(level *l,linedef *ld,style *ThisStyle,quest *ThisQuest,
                   config *c);
slink *random_open_link(level *l,linedef *ld,style *ThisStyle,quest *ThisQuest,
                       config *c);
slink *random_basic_link(level *l,linedef *ld,style *ThisStyle,quest *ThisQuest,
                        config *c);
slink *gate_link(level *l,config *c);
...
linedef *make_linkto(level *l,linedef *ldf,slink *ThisLink,
                              style *ThisStyle,config *c, linedef *old);
void e_bl_inner(level *l,linedef *ldf1,linedef *ldf2,slink *ThisLink,
                quest *ThisQuest,style *ThisStyle,style *NewStyle,
                short flipstate, haa *haa,config *c);
void e_ol_inner(level *l,linedef *ldf1,linedef *ldf2,slink *ThisLink,
                quest *ThisQuest,style *ThisStyle,style *NewStyle,
                haa *haa,config *c);
void establish_basic_link(level *l,linedef *ldf1,linedef *ldf2,slink *ThisLink,
                          quest *ThisQuest,style *ThisStyle,style *NewStyle,
                          haa *haa,config *c);
void establish_open_link(level *l,linedef *ldf1,linedef *ldf2,slink *ThisLink,
                         quest *ThisQuest,style *ThisStyle,style *NewStyle,
                         haa *haa,config *c);
void establish_link(level *l,linedef *ldf1,linedef *ldf2,slink *ThisLink,
                     quest *ThisQuest,style *ThisStyle,style *NewStyle,
                     haa *haa,config *c);
...
boolean link_fitsq(slink *ThisLink,quest *ThisQuest);
boolean link_fitsh(linedef *ldf,slink *ThisLink,config *c);
boolean link_fitsv(level *l,linedef *ldf1,linedef *ldf2,slink *ThisLink);
void Usage0(void);
void Usage(void);
void Usage2(void);
...
config *get_config(int argc, char *argv[])
{
  ...
  if ( (answer->configfile = getenv(SLIGE_CONFIG)) == NULL) {
    answer->configfile = strdup("slige" EXT_SEP "cfg"); /* So's we kin free() it */
	 if(!file_exists(answer->configfile))
		 answer->configfile = strdup(LIBDIR "/slige/slige" EXT_SEP "cfg");
  } else {
...
void dump_link(linedef *ldf1,linedef *ldf2,slink *ThisLink,char *s1)
{
  char s[200];
...
boolean make_window_inner(level *l,linedef *ldf1,linedef *ldf2,slink *ThisLink,
                                 style *ThisStyle,style *NewStyle,config *c)
{
  linedef *ldnew1, *ldnew2;
...
boolean make_window(level *l,linedef *ldf1,linedef *ldf2,slink *ThisLink,
                           style *ThisStyle,style *NewStyle,config *c)
{
...
/* side of this one. */
linedef *make_linkto(level *l,linedef *ld,slink *ThisLink, style *ThisStyle,
...
{
  int dieroll, len = 0;
  slink *answer = (slink *)malloc(sizeof (*answer));
                        ...
                        config *c)
{
  ...
  answer = (slink *)malloc(sizeof (*answer));
...
slink *random_patio_link(level *l,linedef *ld,style *ThisStyle,config *c)
{
  slink *answer = (slink *)malloc(sizeof (*answer));
...
linedef *make_next_room(level *l,sector *oldsector,boolean radical,config *c,
                         linedef **ldf, slink **ThisLink, quest *ThisQuest)
{
  linedef *newldf;
  int i, tries;
...
void NewLevel(level *l, haa *ThisHaa, config *c)
{
   ...
   slink *ThisLink, *ForkLink;      /* Particular instances */

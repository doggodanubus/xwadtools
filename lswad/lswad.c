...
void usage(char *option)
{
	...
	fprintf(stderr, "\t--help or -?: show this help\n");
	...
	fprintf(stderr, "\t-F: list all flats between labels (FF_START|F_START)/F_END\n");
	exit(1);
}
...
static int list_wad(const char *file)
{
	...
	for (i = 0; i < wad_header.numlumps; i++) {
		...
		/* list flats between F_START and F_END? */
		  ...
		  if (!strncmp(&lump.name[0], "F_START", 7) || !strncmp(&lump.name[0], "FF_START", 8)) {
			list_flats = 1;
			continue;
	...
	if(list_flats) {
		fprintf(stderr, "warning: Found flats, but no F_END\n");
	}

...
wadfile_t *open_wad(const char *name)
{
	...
	if(!name) {
		fprintf(stderr, "open_wad(): no such wad file\n");
		exit(1);
	}
	...
	if (fread((void *)&wf->wh, sizeof(wadinfo_t), 1, wf->fp) != 1) {
		fprintf(stderr, "open_wad(): can't read WAD header in %s\n", name);
		exit(1);
	}
	...
	swaplong(&(wf->wh.infotableofs));
        ...
        if (fseek(wf->fp, (long)wf->wh.infotableofs, SEEK_SET) == -1) {
                fprintf(stderr, "open_wad(): can't seek to directory in %s\n", name);
                exit(1);
        }
	...
	for (i = 0; i < wf->wh.numlumps; i++) {
		...
		if (fread((void *)wf->lp->lumps[wf->lp->num_lumps-1],
			...
			fprintf(stderr,
			    "open_wad(): can't read lump directory entry in %s\n\n", name);
			exit(1);
		}

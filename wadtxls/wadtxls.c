...
BCINT Episode    = 1;		/* the episode number */
BCINT Mission    = 1;		/* the mission number */
...
void ParseCommandLineOptions( int argc, char *argv[])
{
   ...
   while (argc > 0)
   {
	...
	if(!MainWad) MainWad = fw_find_iwad();
	if(!MainWad) {
...
void MainLoop()
{
   ...
   if (TxFlName == NULL || strcmp(TxFlName, "-") == 0) {
		TxFlName = "standard output";
		TxFile = stdout;
	}

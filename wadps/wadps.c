...
struct directory *process_wad(char *file, int *fd, long *size);
...
int main(int argc, char **argv)
{
		...
		strcpy(wadfile, path);
                ...
                if((d = process_wad(wadfile, &fd, &size)) != NULL) {
                            ...
                            for(ep=1; ep<=3; ep++)
                                for(lv=1; lv<=9; lv++) {
                                    ...
                                    doit(fd, d, size, objs, mons, deathmatch);
									 ...
									 for(ep=1; ep<=32; ep++) { /* and for doom2 */
										 sprintf(level, "MAP%02d", ep);
										 doit(fd, d, size, objs, mons, deathmatch);
									 }
                            strcpy(level, "");
                        }
...
struct directory *process_wad(char *file, int *fd, long *size)
{
	...
	if ((*fd = fw_open_wad(file, O_RDONLY|O_BINARY, 0)) == -1) {

...
#define RESOURCES_NEEDED ((int)((1<<THINGS)|(1<<LINES)|(1<<SIDES)|(1<<VERTS)|\
...
int dmit(int ver, WAD_THING *things, int nthings, WAD_SECTOR *sects,
         int nsects);
...
int rand_thing(int seed, WAD_THING *things, int nthings);
int flip(WAD_THING *things, int nthings, WAD_VERT *verts, int nverts,
         WAD_LINE *lines, int nlines, WAD_NODE *nodes, int nnodes,
         WAD_SEGS *segs, int nsegs);
int shift(int dx, int dy, int dz, WAD_THING *things, int nthings,
          WAD_VERT *verts, int nverts, WAD_NODE *nodes, int nnodes,
          WAD_SECTOR *sects, int nsects);
  ...
  for (a = 1; a < ac; a++) {			/* scan all arguments */
    if (av[a][0] == '-') {			/* optional argument */
      if (0 == strcmp(av[a],"-?"))
        ...
        strcat(func,"dmit");
      else if (2 == sscanf(av[a],"-rand%1[=:]%d",delim,&seed))
        strcat(func,"rand");
...
#endif
{
  ...
  printf("\tThings........%d: %d %s, %d %s, %d %s, %d %s,\n",
         NThings,creature,"creatures",weapon,"weapons",key,"keys",
         health,"health");
  printf("\t              %*s  %d %s, %d %s (%d bytes)\n",
  ...
  printf("\tLines.........%d: %d 1-sided, %d 2-sided\n",
         NLines,one_sided,NLines-one_sided);
  printf("\t              %*s  %d special, %d tagged (%d bytes)\n",
         (int)(log10((double)max(NLines,1))+1),"",
         special,tagged,dir[THINGS].nbytes);
  printf("\tSides.........%d",NSides);
  if (NSides < 2*NLines-one_sided)
    printf(", packed by %d%%",100-100*NSides/(2*NLines-one_sided));
  printf(" (%d bytes)\n",dir[SIDES].nbytes);
  printf("\tVertices......%d: %d for lines, %d for segs (%d bytes)\n",
  ...
  printf("\tSectors.......%d: %d special, %d secret, %d tagged (%d bytes)\n",
  ...
  for (i = 0; i < NRejects; i++)
    for (j = 0; j < numbits(Rejects[0]); j++)
      if (i*numbits(Rejects[0])+j < NSects*NSects &&
          Rejects[i] & (1<<j)) ones++;
  printf("\tReject........");
  if (ones) printf("%4.1f%% (%d bytes)\n",
                   100.0*ones/(NSects*NSects),dir[REJECTS].nbytes);
  else      printf("zeroed (%d bytes)\n",dir[REJECTS].nbytes);
  ...
  while (list != NULL && 1 == sscanf(list,"%[^,],",mfile)) {
    ...
    for (r = 0; r < winfo->head.count; r++) {	/* search for unique rsrcs */
      ...
      if (e == iwad->head.count) {		/* add in new resource */
       ...
       add_resource(buf,int,iwad->count,iwad->head.count+1,winfo->count[r+i]);
...
#endif
{
  ...
  while (list != NULL && 2 == sscanf(list,"%[^-]-%[^,],",rfile,rrsrc)) {
    ...
    if (e == iwad->head.count) {		/* add in new resource */
      ...
      add_resource(buf,int,iwad->count,iwad->head.count+1,rdir.nbytes);
      add_resource(buf,boolean,iwad->changed,iwad->head.count+1,TRUE);
      iwad->head.count++;

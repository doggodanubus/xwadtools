...
#if defined(MSDOS) || defined(__MINGW32__) /* DJGPP doesn't define it */
...
extern int nodes_make(				/* makes NODES,SSECTORS,SEGS */
#if defined(ANSI_C)
	WAD_NODE **nodes,
	int *nnodes,
	WAD_SSECTOR **ssecs,
...
extern int blockmap_make(			/* makes the BLOCKMAP */
#if defined(ANSI_C)
	short **blockmap,
	WAD_LINE *lines,
	int nlines,
...
extern int reject_make(			/* makes REJECT */
#if defined(ANSI_C)
	WAD_REJECT **reject,
	int rej_flags,
	char *except,
